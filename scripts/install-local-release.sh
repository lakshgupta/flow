...
TARGET_OS="$(normalize_release_os_name "${FLOW_TARGET_OS:-$(uname -s)}")"
TARGET_ARCH="$(normalize_release_arch_name "${FLOW_TARGET_ARCH:-$(uname -m)}")"

if [[ -z "$TARGET_OS" || -z "$TARGET_ARCH" ]]; then
	echo "Could not determine a supported local release target." >&2
	exit 1
fi

if ! release_target_supported "$TARGET_OS" "$TARGET_ARCH"; then
	echo "No local release archive is supported for ${TARGET_OS}/${TARGET_ARCH}." >&2
	exit 1
fi

PROJECT_VERSION="$(project_version "$(project_version_file)")"
DEFAULT_ARCHIVE_PATH="$ROOT_DIR/dist/$(release_archive_name "$PROJECT_VERSION" "$TARGET_OS" "$TARGET_ARCH")"
DEFAULT_CHECKSUM_PATH="$ROOT_DIR/dist/$(release_checksum_name "$PROJECT_VERSION" "$TARGET_OS" "$TARGET_ARCH")"
ARCHIVE_PATH="${FLOW_ARCHIVE_PATH:-$DEFAULT_ARCHIVE_PATH}"
CHECKSUM_PATH="${FLOW_CHECKSUM_PATH:-$DEFAULT_CHECKSUM_PATH}"
...
if [[ ! -f "$ARCHIVE_PATH" ]]; then
	...
	echo "Build it first with: bash ./scripts/build-release.sh ${TARGET_OS} ${TARGET_ARCH}" >&2
	exit 1
fi
