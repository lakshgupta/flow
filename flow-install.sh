#!/usr/bin/env bash
#
# flow-install.sh — Download and install the Flow binary from GitHub Releases.
#
# Usage:
#   ./flow-install.sh               # install the latest release
#   ./flow-install.sh 0.6.0         # install a specific version
#   ./flow-install.sh v0.6.0        # v-prefixed version also accepted
#
# Environment overrides:
#   FLOW_INSTALL_DIR      Destination directory (default: $HOME/.local/bin)
#   FLOW_RELEASE_REPO     GitHub repository in owner/name format (default: lakshgupta/flow)
...
if [[ $# -gt 1 ]]; then
	echo "Usage: flow-install.sh [version]" >&2
	exit 1
fi

if [[ $# -eq 1 && "$1" =~ ^(linux|darwin|amd64|arm64)$ ]]; then
	echo "flow-install.sh installs published releases and expects an optional version such as 0.1.0 or v0.1.0." >&2
	exit 1
fi

usage() {
	cat <<'EOF'
Usage: flow-install.sh [version]
